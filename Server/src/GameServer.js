...
function GameServer() {
    ...
    this.config = {
        ...
        ejectCooldown: 5,           // min ticks between cannon shots (~200ms)
        ejectSpawnPlayer: 0,        // Cannonballs don't spawn players
        
        playerMinSize: 50,          // Minimum ship size
        playerMaxSize: 1500,        // Maximum ship size
        playerMinSplitSize: 9999,   // Disable splitting (ships don't split)
        playerStartSize: 80,        // Starting ship size
        playerMaxCells: 1,          // One cell per player = one ship
        playerSpeed: 1,             // Speed multiplier
        playerDecayRate: .001,      // Very slow size decay
...
GameServer.prototype.resolveCollision = function (manifold) {
    ...
    // ── Pirate: cannonball hits ship ─────────────────────────
    // A cannonball (ejected mass, cellType=3) hitting an enemy ship (cellType=0)
    // deals damage instead of being absorbed normally.
    if (minCell.cellType === 3 && maxCell.cellType === 0) {
        if (minCell.ejector && minCell.ejector.owner !== maxCell.owner) {
            // Damage = 2× cannonball mass-squared removed from ship
            var damageSq = minCell._sizeSquared * 2;
            var newSq = maxCell._sizeSquared - damageSq;
            if (newSq < this.config.playerMinSize * this.config.playerMinSize) {
                // Ship sunk — trigger loot drop via normal removal path
                minCell.isRemoved = true;
                maxCell.isRemoved = true;
                if (maxCell.owner) {
                    var lootCount = Math.max(4, Math.floor(maxCell._size / 25));
                    var lootSize  = Math.max(this.config.foodMaxSize, Math.floor(maxCell._size / 8));
                    for (var li = 0; li < lootCount; li++) {
                        var lootPos = {
                            x: maxCell.position.x + (Math.random() - 0.5) * maxCell._size * 2,
                            y: maxCell.position.y + (Math.random() - 0.5) * maxCell._size * 2
                        };
                        var lootCell = new Entity.Food(this, null, lootPos, Math.min(lootSize, 80));
                        lootCell.setColor({ r: 255, g: 215, b: 0 });
                        this.addNode(lootCell);
                    }
                    // Remove from player's cell list so respawn triggers
                    maxCell.onEaten(minCell);
                }
            } else {
                maxCell.setSize(Math.sqrt(newSq));
                minCell.isRemoved = true;
            }
            return;
        }
    ...
    // ── Pirate loot drop ─────────────────────────────────────
    // When a player ship is sunk, scatter food (loot) around its position
    if (minCell.owner && minCell.cellType === 0) {
        var lootCount = Math.max(4, Math.floor(minCell._size / 25));
        var lootSize  = Math.max(this.config.foodMaxSize, Math.floor(minCell._size / 8));
        for (var li = 0; li < lootCount; li++) {
            var lootPos = {
                x: minCell.position.x + (Math.random() - 0.5) * minCell._size * 2,
                y: minCell.position.y + (Math.random() - 0.5) * minCell._size * 2
            };
            var lootCell = new Entity.Food(this, null, lootPos, Math.min(lootSize, 80));
            lootCell.setColor({ r: 255, g: 215, b: 0 }); // gold colour
            this.addNode(lootCell);
        }
    }
    // ─────────────────────────────────────────────────────────
...
GameServer.prototype.ejectMass = function (client) {
    ...
    for (var i = 0; i < client.cells.length; i++) {
        ...
        // No angle scatter for cannonballs — straight shot
        // Cannon range scales with ship size (bigger ship = longer range)
        var cannonRange = 780 + cell._size * 6;

        // Fire 3 cannonballs in a line from the ship's center, moving sideways
        var perpX = -dy; // broadside axis (perpendicular to fire direction)
        var perpY = dx;
        var spacing = cell._size * 0.40;
        var ballOffsets = [-spacing, 0, spacing];
        for (var bi = 0; bi < 3; bi++) {
            var bpos = {
                x: cell.position.x + perpX * ballOffsets[bi],
                y: cell.position.y + perpY * ballOffsets[bi]
            };
            var ejected = new Entity.EjectedMass(this, null, bpos, size2);
            ejected.ejector = cell;
            ejected.setColor({ r: 20, g: 20, b: 20 }); // dark cannonball colour
            ejected.setBoost(cannonRange, angle);
