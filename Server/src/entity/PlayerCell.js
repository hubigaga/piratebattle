...
PlayerCell.prototype.canEat = function (cell) {
    // ships cannot eat other ships directly — only cannonballs can sink them
    if (cell.cellType === 0) return false;
    // ships can eat food (1) and absorb cannonballs (3) — but cannonball damage
    // is handled separately in resolveCollision, so just allow absorption here
    return true;
};
